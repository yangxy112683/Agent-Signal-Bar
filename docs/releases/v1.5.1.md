...
- Keeps the command line tool consistently named `agent-signal-light`, while preserving `agent-signal` as a compatibility alias.
...
- Stopped treating `gpt-5.3-codex-spark` as a public `$0.00` priced model; Research Preview usage now has an unknown estimated cost instead of appearing free.
...
- 命令行工具统一使用 `agent-signal-light`，同时保留 `agent-signal` 作为兼容别名。
...
- 不再把 `gpt-5.3-codex-spark` 当作公开 `$0.00` 定价模型；Research Preview 用量会显示为未知费用，而不是免费。
